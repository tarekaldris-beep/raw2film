...
@group(0) @binding(4)
var<uniform> params : HalationParams;
...
fn highlight_mask(luma : f32) -> f32 {
    let normalized = luma / max(params.white_luma, 1e-6);
    let x = (normalized - params.threshold) / max(params.softness, 1e-6);
...
fn main(
    ...
    for (var ky: i32 = 0; ky < k_size.y; ky++) {
        ...
        for (var kx: i32 = 0; kx < k_size.x; kx++) {
            ...
            // Only highlights emit; the kernel is an untinted blur.
            let mask_val = highlight_mask(luminance(neighbor.rgb));
            halo += neighbor.rgb * mask_val * weight.rgb;
            weight_idx++;
        }
    }

    // Warm glow, normalized by the glow's luminance (a single scalar) so the
    // tint stays warm and highlights never shift toward blue/cyan.
    let factors = params.intensity * vec3<f32>(params.red, params.green, params.blue);
    let glow = halo * factors;
    let glow_luma = luminance(glow);
    let result = (center.rgb + glow) / (1.0 + glow_luma);
